...
namespace _2.BUS.Services
{
    public class CuaHangServices : ICuaHangServices
    {
        ...
        public List<CuaHang> GetAll(string input)
        {
            ...
            return _icuaHangReps.GetAll().Where(c => c.Ten.ToLower().StartsWith(input.ToLower()) || c.Ma.ToLower().StartsWith(input.ToLower())).ToList();
        }

        public IEnumerable<object> LocTrangThai(int input)
        {
            return _icuaHangReps.GetAll().Where(c => c.TrangThai == input).ToList();
        }
